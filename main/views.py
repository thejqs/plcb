...
class UnicornView(View):
    ...
    def top_10_price_minus_sale_price(self, lst_of_objs):
        """
        given a list of Django objects, each of which contains 'price' and 'on_sale_price'
        attributes, finds the largest gaps between retail price and sale price --
        in other words, the most deeply discounted products in real dollars
    ...
    def get(self, request):
        ...
        # fp, unicorns_json = self.choose_correct_file()
        ...
        print unicorns_dict['scrape_date']
    ...
    def post(self, request):
        ...
        print form.is_valid()
        if form.is_valid():
            search = '\'%%{}%%\''.format(form.cleaned_data['name'])
            print search
            try:
                context['unicorn_response'] = Unicorns.objects.raw("SELECT id, name, price FROM main_unicorns WHERE name ILIKE {} AND scrape_date = \'{}\' ORDER BY price DESC".format(search, str(today.strftime('%Y-%m-%d'))))
            except:
                context['unicorn_response'] = Unicorns.objects.raw("SELECT id, name, price FROM main_unicorns WHERE name ILIKE {} AND scrape_date = \'{}\' ORDER BY price DESC".format(search, str(yesterday.strftime('%Y-%m-%d'))))
            # print context['unicorn_response']
